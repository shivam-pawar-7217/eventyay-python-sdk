...
class TestAttendeesAPI(unittest.TestCase):
    ...
    def test_get_event_attendees(self, mock_get):
        ...
        mock_response.json.return_value = {
            "data": [self.mock_attendee_data, {"id": 2, "email": "jane@example.com", "firstname": "Jane", "lastname": "Smith", "isCheckedIn": True}],
            "meta": {"total": 2}
        }
        mock_get.return_value = mock_response

        # Call the method
        attendee_list = self.client.get_event_attendees("test-event", page=1, page_size=10)
    ...
    def test_get_attendee(self, mock_get):
        ...
        mock_get.assert_called_once_with(
            "https://dev.eventyay.com/api/v1/events/test-event/attendees/1", 
            params=None
        )
